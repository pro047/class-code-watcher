...
### (카) `noise-filter` 주행 — ✅ **완주** (2026-09-06, `965e7f7`·`bcd440e`). F6·F7 이 닫혔다

HANDOFF 5절 (차) 의 **F6**(포맷터 노이즈)·**F7**(atime 오탐)이 PRD v1.8 의 C-24·C-25 로
올라가고, 주행 중 발견한 것이 v1.9 의 C-26 이 됐다. 게이트 **452 → 490 passed**.

#### 비용 — 완주분보다 버린 것이 더 비쌌다

| 계보 | design | judge | impl | verify | 소계 |
|---|---|---|---|---|---|
| 1차 (09-04) | $6.25 / 49턴 | $2.95 / 42턴 (한도) | — | — | **$9.20** |
| 2차 (09-05) | $3.67 / 31턴 | $5.56 / 58턴 | $0.38 / 7턴 (BLOCKED) | — | **$9.61** |
| **3차 (09-05~06)** | **$4.97 / 45턴** | **$6.24 / 57턴** | **$2.30 / 52턴** | **$7.38 / 60턴** | **$20.89** |
| | | | | **총합** | **$39.71** |

**버려진 $18.81 중 $9.61 은 낭비가 아니다** — 그것이 C-26 을 만들어 냈다. 1차의 $9.20 만
순손실이다(계정 세션 한도).

#### 1차 사망 — 세션 한도는 폴백 체인이 못 구한다

judge 에서 `claude-fable-5-1` → `claude-opus-5` → `claude-sonnet-5` 세 모델이 **전부** 같은
`You've hit your session limit` 을 받았다. 모델 폴백은 과부하·부재를 위한 것이지 **계정 단위
한도**를 위한 것이 아니다.

**폴백은 공짜가 아니다.** 1순위 fable 이 **42턴 · $2.95** 를 쓴 뒤 걸렸고, 그 뒤 두 모델이
각 1턴 $0 로 거부됐다(`judge.ratelimit1/2.result.json`). 손실은 그때까지 쓴 값이지 0 이 아니다.
이 문서가 한때 "$0 에 죽었다"고 적었는데 최종 `result.json` 만 읽고 `ratelimit1/2` 를 안 읽은
탓이었다 — `7c06e77` 에서 정정했다.

#### 2차 BLOCKED — judge #37 이 잡고 impl 이 멈췄다. 게이트가 일한 자리다

2차 설계가 `no_change = hash_no_change or is_diff_empty(diff_result)` 를 썼다. judge 가
**반박**으로 올렸다: 그 조건식이 FR-025 와 무관한 **binary·too_large-only 세션까지** 삼켜
`tests/test_watcher.py` 의 「사실만 기록한다」 결정(주석 「설계 5.6」)을 언급 없이 뒤집는다.

impl 은 코드를 **한 글자도 쓰지 않고** `STATUS: BLOCKED` 를 올렸다 — "설계가 틀렸으면 고치지
말고 BLOCKED" 규칙대로다. 7턴 $0.38 에 멈췄다.

> **여기가 이 파이프라인이 값을 한 자리다.** 잘못된 전제 위에 764줄이 쌓이기 전에 멈췄다.

#### 사람 결정 → `DECISION.md` → 재설계

A(확장) / B(좁힘) / C(재설계) 를 놓고 런처가 **D** 를 제안했다 — 요약 생략과 `no_change` 값을
분리한다. 사람이 D 를 골랐고, 런처가 D 를 "한 줄 수정"이라고 말했다가 **끝까지 추적하지 않은
것을 발견해 정정했다**: `resolve_session_end` 에 갈래가 없어 정상 세션이
`PARTIAL / discord_not_attempted` + 종료 코드 1 로 끝난다. 그래서 손수정 대신 재설계로 갔다.

`.pipeline/noise-filter/DECISION.md` 를 만들고 `prompts/design.md` 의 입력에 넣었다(`7c06e77`).
**설계 단계는 `DECISION.md`·`IMPL.md`·`JUDGE.md` 를 이제 물려받는다** — 그전에는 `PRD.md` 와
`src/` 만 읽어서 이전 시도가 왜 멈췄는지 몰랐다.

#### 3차 완주 — 새 게이트 둘이 실제로 작동했다

정본에서 받아온 `TEST_FILES` 지문 게이트가 **양방향으로** 걸렸다:

```
✔ impl 쓰기 권한 준수 (테스트 파일 미변경)
✔ verify 쓰기 권한 준수 (소스 파일 미변경)
```

impl 은 `DESIGN.md` §3 이 테스트 4개도 "변경 대상"으로 나열했는데 **거부했다** — 이 단계의
규칙이 금지하기 때문이다. 그리고 pytest 를 직접 돌려 실패 1건(`test_crlf_only_change_yields_empty_diff`)을
확인한 뒤 BLOCKED 가 아니라 **인계**로 넘겼다(세 조건: 소스 완결 / mypy·ruff 녹색 / 붉은 원인이
`tests/` 안에만). verify 가 그 테스트를 **지우지 않고 고쳤다**.

#### 런처의 실수 하나 — 범위 게이트가 잡았다

3차 첫 실행이 `exit 2 / scope creep (judge) / prompts/design.md` 로 죽었다. 런처가 프롬프트를
worktree 에 **`cp` 로 복사만 하고 커밋·머지를 안 해서** 그 파일이 "커밋 안 된 변경"으로 남았다.
게이트는 정확히 판정했다 — 다만 그 변경을 만든 게 에이전트가 아니라 런처였다.
`git checkout -- ` + `merge --ff-only` 로 해소했고 **design·judge 산출물은 재사용돼 $11.21 을
다시 쓰지 않았다.**

#### judge 의 `미확인` 2건은 pytest 가 닫았다

`python -c` 도구 거부로 judge 가 실측 못 한 것 둘 — ① `difflib.unified_diff(n=0)` 이 동일
시퀀스에 빈 이터레이터를 내는가 ② `str.split()` 의 유니코드 공백 범위. `DECISION.md` 가
"구현 단계 pytest 로 고정한다"로 정했고 실제로 그렇게 닫혔다. `tests/test_diffgen.py:621` 이
**ZWSP(U+200B)는 남는다**는 필터의 실제 경계를 단언한다.

#### 실기기 대조 (2026-09-06, macOS) — 같은 입력을 개정 전/후로

| 시나리오 | 개정 전 | 개정 후 |
|---|---|---|
| 공백 전용 (재들여쓰기) | `files_changed 1 / +5 / -5 / partial` · 정제 실행 · **요약 시도** | `files_changed 0 / +0 / -0 / completed` · 정제 안 돎 · **요약 0회** |
| 같은 내용 2회 재저장 | `events 2` · `events.jsonl` 2줄 | `events 0` · **`events.jsonl` 미생성** |
| 재들여쓰기 6줄 + 진짜 1줄 | — | `final.diff` **-5/+6 전부**, `change_stats` **+1/-0** |

세 번째가 D2 결정(본문은 전부, 숫자는 의미 있는 것만)이 실물로 확인된 자리다.

**개정 전에는 요약을 시도했다** — API 키가 있었다면 포맷터 결과만 든 diff 로 OpenAI 를 불렀다.
C-24 가 겨냥한 그 경로다.

> **부수 발견 (제품 결함 아님)**: `&` 로 백그라운드에 띄우면 Ctrl+C 가 안 먹는다. 비대화형 셸이
> 자식에게 SIGINT 를 `SIG_IGN` 으로 물려주는 POSIX 동작이다. 실수업은 터미널 전경 실행이라
> 무관하지만, **7단계에서 exe 를 배치·서비스로 띄울 계획이 생기면 이 자리가 걸린다.**

#### 남은 것 — 사람만 할 수 있다

`VERIFY.md` §5.2 의 H1~H6. 요지: **H1** Windows 실기기에서 읽기만 했을 때 `events.jsonl` 이
안 생기는가(C-25 원본, macOS 재현 불가) · **H2** VS Code + prettier 로 C-24 재현
(**89,101자가 0 이 되지는 않는다** — 줄 접힘은 여전히 `modified`) · **H6** `no_change: false` 인데
`discord.skip_reason: "no_change"` 인 조합을 사람이 모순으로 읽는지. **H6 이 C-26 의 유일한
미검증 표면이다** — 모순으로 읽힌다면 `notify.py` 에 6번째 사유 상수를 넣을 시점이다.
