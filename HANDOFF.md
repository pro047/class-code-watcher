...
- **지금 열려 있는 제품 이슈 — 모델이 diff 밖 일반론을 채운다.**
  2026-08-31 두 실세션에서 같은 패턴이 나왔다:
  `복습할 질문` 에 "로컬 스토리지와 세션 스토리지의 차이"(코드에 없음),
  `학습 포인트` 첫 항목이 **"OAuth2 이해하기"**(코드에 OAuth2 가 한 줄도 없음).
  원인 추정: 08-30 에 넣은 "questions_to_review 를 비워 두지 않는다 / 1~5개로 반드시
  채운다"가 **채우라는 압박**으로 작용한다. 배열 상한(5)까지 채우려다 근거 밖으로 나간다.
  **`confidence` 필드가 이미 있는데 전 항목이 `high` 로만 나온다** — 근거 없는 항목까지
  `high` 다. 여기가 고칠 자리다. 이건 `summarize.py` 프롬프트 문안이라 **4단계 영역**이다.
