...
- 기준 문서: `PRD.md` **v1.6** (14절 MVP 단계별 개발 계획, C-17·C-18·C-19·C-20·C-21 반영)
- 갱신 시점: 2026-09-02
...
- 한 줄 요약: **0~5단계 완료. C-17(메시지 형식)·C-18(하루치 예산)은 코드에 들어갔고,
  ...
  **다음 작업 순서 — 2026-09-02 사람이 갱신했다. (1) 이 최우선이다**:
  (1) **★ C-19 주행 재개 — `JUDGE.md` 게이트에서 승인 대기 중이다** (`summarize.py`·`notify.py`).
  design·judge DONE, impl 은 아직 시작 전이다 — 5절 (아) 「2차 주행」.
  **구현을 시작하기 전에 사람이 정할 것이 3건 있다. impl 이 돌기 시작하면 셋 다 설계부터 다시다:**
  **① judge 반박 #28** (`MAX_TERM_CHARS` 40→32 의 근거 수치 오류. 결론은 유효 — 권고는 그대로 승인),
  **② F14 — OpenAI 타임아웃 15초가 C-18 예산과 어긋난다** (오늘 실제로 세션을 죽였다. 5절 (자)),
  **③ F15 — 15개 상한으로는 오늘 같은 API 열거 수업을 못 담는다** (묶기 방식 실측 완료. 5절 (자)).
  ②③ 은 PRD 개정이 먼저다 — 설계가 PRD 만 읽기 때문이다.
  산출물 사본은 **`docs/pipeline-transfer/dynamic-groups/`** 에 커밋돼 있다 — 다른 PC 에서
...
| **F14** | **타임아웃 15초가 C-18 예산(60,000자)과 어긋난다.** 09-02 세션을 실제로 죽였다 (`partial`) — 5절 (자) | `openai_client` + PRD 7절 |
| **F15** | **F13 을 고쳐도 15개 상한이 막는다.** API 열거 수업은 68종 이상이다. 묶기 방식 실측 완료 — 5절 (자) | `summarize` + PRD 11.2·11.3 |
...
**고칠 곳**: `openai_client.py:34` 의 `OPENAI_TIMEOUT_S = 15.0`, 그리고 **PRD 7절 비기능 요구의
두 문구**(타임아웃 15초 · 종료 30초 내). 예산 60,000자를 다 쓰는 세션이 기준이므로 **60초 이상**이
맞다. **PRD 를 먼저 고쳐야 한다** — 설계 에이전트는 PRD 만 읽고, 지금 PRD 는 15초가 정본이다.
...
**사람이 정할 것**: F15 를 C-19 주행에 얹을지(=PRD 11.2·11.3 개정, C-22), 아니면 C-19 를 먼저
닫고 다음 주행으로 미룰지. **얹는다면 지금이 가장 싸다** — impl 이 아직 시작 전이라 설계만
다시 뽑으면 되고, impl 이 돈 뒤에는 한 바퀴($13 안팎)를 더 태운다.
