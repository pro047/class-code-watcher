...
**2단계는 실기기 확인이 통째로 미착수다** — 5건 전부 (5절 가).
...
**2단계 실기기 확인 5건 — 전량 미착수.** `VERIFY.md` 5절이 남긴 것이다.
자동 테스트는 스냅샷 바이트만으로 돌아 실기기를 한 번도 안 탔다:
...
2. ⬜ **실제 IDE 저장 세션의 diff 가독성** — 2~3곳 고치고 종료. `final.diff` 가 사람 눈에
   읽히는지, 콘솔에 `[DIFF] N개 파일 변경 (+a / -d)` 한 줄이 떴는지
3. ⬜ **1MB 근처 파일 체감** — 종료 지연이 느껴지는지, `# skipped: … (too_large)` 로 남는지
4. ⬜ **cp949 콘솔의 `[DIFF]` 줄** — 한글이 안 깨지는지. (다)에서 좁힌 조건대로
   **`> log.txt` 로 리다이렉트했을 때**가 진짜 위험 구간이다
5. ⬜ **`stats.json` 육안 대조** — impl 이 "본문을 눈으로 확인하지 못했다"고 남겼다.
   실기기 세션 1회의 `totals`·`files` 를 실제 편집 내용과 맞춰본다
