...
| | 무엇을 | 닫히는 것 |
|---|---|---|
| ~~**H1**~~ | ~~`summary.json` 의 `truncated` 가 `false`~~ | **✅ 닫혔다 (2026-09-08). 답은 「아니다」다** — 하루 8시간 세션(JS DOM)이 `final.diff` **108,957자 = 예산 60,000 의 181.6%**, `truncated: true`. C-27 로 예산을 **110,000** 으로 올렸고, 같은 세션이 그 값에서 `truncated: false` 다(실측 재현). 더 큰 것을 함께 잡았다 — 아래 **B-i** |
...
| **B-ii** | 예산 60,000 이 하루치에 모자랐다 (H1) | 110,000. 실측 108,957자에 여유를 얹지 않았다 |

**B-i 이 셋 중 제일 크다.** 예산만 올리는 것으로는 안 된다 — 같은 diff 를 80,000~100,000 으로
돌리면 이번에는 2순위가 통째로 누락된다(실측). 배분을 안 고치면 예산을 얼마로 잡든 어느 한쪽이
굶는다.
...
**남은 위험 — 타임아웃을 다시 재야 한다.** C-22 의 90초는 입력 34,294자에서 실측된 값이다.
110,000자를 다 쓰는 세션은 그 3.2배이고 **아직 실측이 없다.** 다음 대용량 세션에서
`error: openai_timeout` 이 나면 C-22 와 같은 방식으로 값을 올린다 — 로직이 아니라 값 문제다.
