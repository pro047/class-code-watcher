...
- 게이트 3종 녹색: **502 passed** / ruff clean / mypy 17 files
...
- 한 줄 요약: **0~5단계 완료. C-17~C-26 이 전부 코드에 들어갔다**
  (`dynamic-groups` 2026-09-03 `3df4f36` + `noise-filter` 2026-09-06 `bcd440e`).
  **아직 실 API 를 한 번도 안 탔다** — 490개가 전부 가짜 `CallFn`·가짜 sender 주입이다. C-19 문안 전체와 C-23 의
...
| (카) | **`noise-filter` 주행 — F6·F7 → PRD v1.8 (C-24·C-25) + v1.9 (C-26)** (`bcd440e`) | 09-06 |
...
| | 무엇을 | 닫히는 것 |
|---|---|---|
| ~~**H1**~~ | ~~`summary.json` 의 `truncated` 가 `false`~~ | **✅ 닫혔다 (2026-09-08). 답은 「값은 맞다, 근거가 틀렸다」다** — 하루 8시간 세션(JS DOM)이 `final.diff` **108,957자 = 예산의 181.6%**, `truncated: true`. 그래서 110,000 으로 올렸다가 **한 요청도 못 통과했다(HTTP 429 ×3)**. 상한을 정하는 것은 diff 크기가 아니라 **계정 TPM 30,000** 이었다 — 110,000 은 약 40,000 토큰이라 버킷 자체를 넘는다. **예산 60,000 은 유지한다.** `truncated: false` 는 이 계정에서 하루치로는 달성할 수 없는 목표다. 실제로 고친 것은 아래 **B-i** |
| **H3** | 모바일에서 `[근거 일부 누락 …]` 이 첫 화면에 보이는가 | F2 (절단 표시). **2026-09-08 실전송본에서 3번째 줄에 떴다** — 데스크톱 확인이고, 모바일 첫 화면 여부는 아직 사람이 안 봤다 |
| **H4** | 키워드 10건 이상 실전송에 Discord 가 400 을 안 준다 | `DISCORD_CONTENT_LIMIT=2000`. 09-08 은 8건이라 못 닫는다 |

**H2 는 09-01 에 닫혔다** (hunk 경계 절단 — 중간에서 잘린 hunk 0건).
**H1·H3·H4 는 사람 확인 C(모바일)·D(수신자 5인)와 같은 자리에서 한 번에 본다** —
따로 만들려고 팀 채널에 합성 메시지를 쏘지 마라.

**2026-09-08 JS DOM 세션이 닫은 것 — C-27 (파이프라인 밖, 사람 결정)**

실수업 세션 하나가 결함 3건을 동시에 드러냈다. 셋 다 게이트가 못 잡는 종류다 — 테스트는 전부
녹색인데 실전송본이 잘못 나갔다.

| | 무엇이 틀렸나 | 고친 것 |
|---|---|---|
| **A** | `term`·`syntax` 를 글자 수로 자르니 C-23 계열 목록이 반토막 났다 — 실전송본 `setInterval·clea` | `_truncate_at_boundary` 로 구분자 경계까지 물러난다. 상한값은 불변이라 C-23 관계식도 불변 |
| **B-i** | `build_prompt` 가 **변경량 우선순위를 배분에서 뒤집었다.** 1순위(961줄) 4/14 hunk, 2순위(381줄) 전량 — PRD 11.1 원칙 6 과 어긋난다 | C-18 의 2패스를 한 패스로 되돌렸다. **예산 60,000 그대로도 1순위가 4/14 → 12/14** |
| **B-ii** | 예산 60,000 이 하루치에 모자랐다 (H1) | **되돌렸다 — 60,000 유지.** 110,000 은 HTTP 429 로 한 번도 안 통과했다. 상한은 계정 TPM(30,000)이 정한다 |

**B-i 이 셋 중 제일 크다 — 그리고 유일하게 살아남았다.** 예산은 올릴 수 없다(TPM). 같은 diff 를
80,000~100,000 으로 돌리면 이번에는 2순위가 통째로 누락되기도 한다(실측). **예산을 못 건드리는
이상 배분이 전부다** — B-i 하나로 1순위가 4/14 → 12/14 가 됐다.

**TPM 실측 (2026-09-08, `x-ratelimit-*` 헤더)**

| | 값 |
|---|---|
| `x-ratelimit-limit-tokens` | **30,000** / 분 |
| 밀도 (실측) | **2.77 자/토큰** (54,667자 → 입력 19,734 토큰) |
| 예산 60,000 짜리 호출 1회 | 입력 **19,734 토큰** = 버킷의 66% |
| 예산 110,000 이면 | 약 **40,000 토큰** — 버킷 초과, **재시도로도 못 통과** |

**지뢰**: FR-030 의 재시도 1회가 같은 분에 일어나면 19,734 × 2 = 39,468 > 30,000 이라 **재시도가
429 로 죽는다.** 재시도를 살리려면 예산이 약 40,000 이어야 한다. 스키마 실패가 드물어 받아들인
대가이고, 잦아지면 그때 내린다. **연속 호출도 마찬가지다** — 같은 세션을 재요약하려면 버킷이
찰 때까지(약 60초) 기다려야 한다.

**C-18 이 2패스로 막으려던 것은 이제 일어난다** — 작은 파일이 통계만 남을 수 있다. 없어진
위험이 아니라 받아들이는 대가이고, 그 근거는 PRD C-27 에 있다. 테스트 `A8` 은 삭제하지 않고
**뒤집힌 기대값으로 다시 썼다** (`test_the_biggest_file_claims_its_hunks_before_smaller_files_are_loaded`).

**타임아웃은 위험이 아니었다.** 예산을 못 올리므로 C-22 의 90초가 겪을 입력 크기도 안 바뀐다.
09-08 재요약 실측은 입력 19,734 토큰에 응답까지 수 초였다.

**파이프라인 밖에서 고쳤다.** 6절의 「1단계부터는 파이프라인 안에서 닫는다」에 대한 두 번째
예외이고 **사람이 결정했다 (2026-09-08)**. 선례는 09-06 의 D11 정정이다. 회귀 테스트 5개를
먼저 넣고 **고치기 전 코드에서 4개가 실패하는 것을 확인**했다 (`main.html` 이 정확히 4/14 로
실세션과 일치). 게이트 **492 → 497 passed** / ruff clean / mypy 17 files.
...
| ~~**F17**~~ | ~~C-24 가 파일 단위라 공백 노이즈가 줄 단위로 살아남는다~~ **✅ 닫혔다 (2026-09-09, C-28).** `unified_diff_text` 가 공백 지운 키로 짝을 맞추고 원본 줄을 싣는다(`git diff -w`). 실측 **69,752 → 55,420자(21% 감소), 예산의 116% → 92% 로 전량 수록** | `diffgen` |

> **F6·F7 은 ✅ 닫혔다** (2026-09-06 `noise-filter`, `bcd440e`). PRD v1.8 의 FR-025(C-24)·
> FR-018(C-25) 로 올라갔고 주행 중 나온 것이 v1.9 의 C-26 이다. 주행 기록은 JOURNAL (카).
> **남은 것은 실기기 확인뿐이다** — 아래 「사람만 닫을 수 있는 것」의 N2 (N1·N3 은 닫혔다).
>
> **한계를 알고 있어라**: 대소문자(`<!DOCTYPE>`→`<!doctype>`)·자기닫힘 태그·줄 접힘은
> 공백이 아니라 **안 걸러진다.** 09-01 오후의 89,101자가 0 이 되지는 않는다.

**F17 — 오늘 잘린 진짜 원인은 수업량이 아니라 포맷터였다 (2026-09-09 실측)**

> **C-27 을 고치고도 오전 세션이 잘렸다.** 그래서 팠고, 원인이 예산도 배분도 아니었다.

2026-09-09 오전 세션(`20260909-091209-e7d3`, `15_문서객체모델_DOM조작.html` 한 파일):

| | 값 |
|---|---|
| baseline → final 파일 크기 | 42,927자 → 49,502자 (**실제 순증 +6,575자**) |
| `final.diff` | **69,752자 — 순증의 10.6배** |
| 그중 공백 차이가 만든 부피 | **32,599자 (47%)** |
| 내용 같고 공백만 다른 줄 | `+`/`-` 양쪽에 **283줄씩** |
| C-28 적용 후 `final.diff` | **55,420자 (예산의 92%) — 전량 수록** |

**고친 뒤 실측: 69,752 → 55,420자. 그 세션은 안 잘린다** — 예산의 92% 로 전량 들어간다.
(처음 어림한 37,153자는 공백을 **아예 지운** diff 였다. C-28 은 원본 줄을 싣기 때문에
그보다 크다 — 줄어드는 것은 21% 다.) 실제 예시:
...
**왜 C-24 가 못 걸렀나 — 판정 단위가 파일이라서다.** FR-025 는 「한 파일의 변경 줄이
**전부** 공백 전용이면 그 파일을 제외한다」이고, 진짜 수업 내용이 함께 바뀐 파일은
제외 대상이 아니다. 그래서 같은 파일 안의 공백 노이즈가 통째로 딸려 들어간다.
**C-24 가 틀린 것이 아니라 범위가 좁다.**

**이미 코드 안에 답이 반쯤 있다.** `diffgen.normalized_lines` 가 바로 그 일을 하고,
`stats` 는 그것으로 센다 — 그래서 같은 세션이 `stats` 로는 `+668/-264` 인데
`final.diff` 에는 `+935/-562` 줄이 들어 있다. **세는 쪽과 LLM 에 보내는 쪽이 다르다.**
이 불일치가 F17 의 정체다.

**✅ 닫혔다 (2026-09-09, C-28 · 파이프라인 밖, 사람 결정).** `unified_diff_text` 가
`difflib.SequenceMatcher` 를 **공백 지운 키**로 돌리고 출력은 **원본 줄**을 싣는다 —
`git diff -w` 와 같은 규칙이다. 공백을 지운 채로 실으면 모델이 읽는 것이 코드가 아니게
되므로 키와 출력을 나눈 것이 핵심이다. 문맥 줄은 after 쪽을 실어 파일의 현재 모습을 준다.

FR-025 ③ 이 「평소대로 전부 생성한다 — 부분 삭제하지 않는다」였으므로 **PRD 개정이
따라붙었다 (v1.10 → v1.11)**. 그 조항의 가드 테스트
(`test_mass_reindent_with_one_real_change_keeps_full_body_but_counts_one`)는 삭제하지 않고
**뒤집힌 기대값으로 다시 썼다** — C-18 의 A8 을 C-27 이 다룬 방식과 같다.

**파이프라인 밖에서 고쳤다 — 6절 예외 세 번째다.** 선례는 09-06 D11, 09-08 C-27.
회귀 테스트 5개를 먼저 넣고 고치기 전 코드에서 3개가 실패하는 것을 확인했다.
게이트 **497 → 502 passed** / ruff clean / mypy 17 files.

**통계와 본문이 정확히 일치하지는 않는다.** `normalized_lines` 는 빈 줄을 버리고 본문은
코드 모양 때문에 싣는다 — 실측 통계 `+668/-264` vs 본문 `+666/-293`(빈 줄 +8/-39).
의도된 차이이고, 테스트 이름도 「같아진다」가 아니라 「같은 기준으로 접힌다」로 적었다.

**주의 — 다 없애지는 못한다.** C-24 근거 문단이 이미 적었다: 대소문자
(`<!DOCTYPE>`→`<!doctype>`)·자기닫힘 태그·**줄 접힘**은 공백이 아니라 그대로 남는다.
**21%가 100%가 되지 않는다** — 09-09 오전 세션도 여전히 예산의 92%다. 여유가 8%뿐이라
조금 더 긴 수업이면 다시 잘린다.

**노이즈는 파일당 한 번뿐이다 (2026-09-09 실측).** prettier 가 손으로 쓴 파일을 **처음**
만질 때만 전체를 재들여쓰기하고, 그 뒤로는 이미 자기 스타일이라 공백을 안 건드린다.
같은 날 오후 세션(같은 파일, `formatOnSave` 켠 채)에서 **공백만 다른 줄 0개**로 확인했다.

| | 공백만 다른 줄 |
|---|---|
| 오전 — 손으로 쓴 상태 → prettier 첫 통과 | **283줄** |
| 오후 — 이미 포맷된 상태 → 이후 저장 | **0줄** |

그래서 **C-28 과 별개로 운영 회피책이 여전히 유효하다**: 수업 시작 전에 폴더 전체를 한 번
저장해 포맷을 정착시킨 뒤 `cw` 를 띄운다. C-28 은 그것을 잊었을 때와 **새 파일이 수업 중에
처음 포맷되는 날**을 막아준다.
...
**`noise-filter` 가 남긴 실기기 확인 3건** — VERIFY.md §5.2 의 H1·H2·H6 이다. (차) 의 H1·H3·H4 와 번호가 겹쳐서 여기서는 **N1·N2·N3** 으로 부른다:

| | 무엇을 하면 | 무엇이 보여야 하나 |
|---|---|---|
| ~~**N1**~~ | ~~Windows 실기기에서 감시 중 대상 파일을 **열어 읽기만** 한다 (저장 금지)~~ | **✅ 통과 (2026-09-07, Windows PC `ksmart`).** 파일 3개(`.html`·`.js`)를 baseline 뒤에 Python 읽기·`type`·PowerShell `Get-Content`·VS Code 열기로 4회 읽기만 하고 Ctrl+C. 콘솔 `변경 감지` **0줄** · `events.jsonl` **미생성** · `change_stats.events == 0` · `no_change: true` · `status: completed` · 종료 코드 **0**. **전제가 실제로 성립했다** — `fsutil behavior query disablelastaccess` = 2(갱신 켜짐)이고 세 파일의 LastAccessTime 이 세션 시작 08:56:24 뒤인 08:56:33 으로 바뀌었다. 즉 09-01 의 atime 17건을 만든 갱신이 이번에도 일어났고 워처가 그것을 논리 이벤트로 올리지 않았다. FR-018(C-25) 의 첫 Windows 실측. 방법은 (가) 머리의 콘솔 붙이기(AttachConsole → CTRL_C_EVENT) 그대로 |
...
**N3 은 닫혔고 결과는 「D11 이 틀렸다」다. N1 은 2026-09-07 Windows 실측으로 닫혔다.** 남은 실기기 확인은 **N2** 하나다.
...
**운영: 수업을 오전/오후 두 세션으로 나눈다 (2026-09-09, 사람 결정)**

> **C-18 의 「요약 단위를 하루 1세션으로 확정」을 운영에서 되돌린 것이다.** 코드는 안 바뀐다 —
> 워처는 자기가 감시한 세션을 요약할 뿐이고, 바뀌는 것은 사람이 몇 번 띄우느냐다.
> **C-18 을 몰라서 나눈 것이 아니다.** 다음 세션이 "하루 1세션이 정본인데 왜 나눠 쓰지"
> 하고 되돌리지 마라.

C-27 이 밝힌 TPM 천장(30,000) 때문이다. 하루치 diff 를 한 요청에 담을 수 없고, 예산을
올려서 해결할 수도 없다. 나누면 **적어도 오전이 통째로 사라지는 일은 막는다.**

```powershell
cw "JS 브라우저객체"    # 오전. 점심때 Ctrl+C
cw "JS DOM"             # 오후. baseline 이 오전 종료 상태로 잡혀 중복이 없다
```

**제목을 반드시 넘겨라.** 생략하면 기본값 `JS 수업` 이 그대로 박히고, 두 세션이 같은
제목으로 나가 수신자가 구분할 수 없다.

**나누기의 한계 — 오후는 여전히 잘릴 수 있다.** 2026-09-08 을 갈랐다면:

| | final.diff | 예산 대비 | |
|---|---|---|---|
| 오전 `13_브라우저객체.html` | 35,526자 | 59% | 전량 ✅ |
| 오후 `14_문서객체모델_DOM기초.html` | 73,431자 | 122% | **12/14 hunk — 여전히 잘림** |

즉 **나누기는 오전을 되찾는 방법이지 오후를 고치는 방법이 아니다.** 오후까지 온전히
담으려면 계정 티어를 올리는 수밖에 없다 (`personal-wfosyz`, 현재 30,000 TPM / 500 RPM,
platform.openai.com → Settings → Limits).

**운영 회피책** — 2026-09-06 `noise-filter` 로 **두 항목의 성격이 바뀌었다.** 지우지 않고
상태를 적는다 (실기기 N2 로 확인한 뒤에 정리한다 — N1 은 2026-09-07 에 닫혔다).

- **[완화됨, 해결 아님]** 수업 시작 전에 전체 파일을 한 번 저장해 포맷을 정착시킨 뒤 watcher 를
  띄운다. 또는 수업 폴더의 `formatOnSave` 를 끈다.
  → FR-025(C-24)가 **공백 전용 변경은 이제 걸러낸다.** 하지만 대소문자(`<!DOCTYPE>`→`<!doctype>`)·
  자기닫힘 태그·**줄 접힘**은 공백이 아니라 그대로 남는다. **09-01 오후의 89,101자가 0 이 되지
  않는다** — 얼마나 줄었는지는 N2 로 재야 안다. 그때까지 이 회피책은 유효하다.
- **[해결됨]** ~~세션이 도는 동안 감시 루트를 읽지 마라~~ (F7 → FR-018/C-25).
  읽기만 한 알림은 이제 논리 이벤트로 기록되지 않는다. 실측(2026-09-06 macOS): 같은 내용
  2회 재저장에 `events 0` · `events.jsonl` 미생성 (개정 전에는 `events 2` · 2줄).
  ~~다만 NTFS atime 오탐 자체는 macOS 에서 재현 불가라 N1 이 남아 있다.~~ **→ N1 도 2026-09-07 Windows 에서
  닫혔다** — atime 갱신이 실제로 일어난 상태(LastAccessTime 변경 확인)에서 `events 0` · `events.jsonl` 미생성.
...
| C-26 | **「의미 있는 변경 없음」은 `no_change` 와 다른 축이다.** diff 대상 0개면 요약·전송·정제를 생략하되 `no_change` 는 **해시 기준 그대로** (2026-09-05, 사람 결정) | PRD 12절 표(`:789`)가 「변경 없음 = baseline/final hash 전부 동일」로 이미 정의하고 있었다 — `no_change` 를 확장하면 **파일이 실제로 바뀐 세션을 「변경 없음」으로 기록**하게 된다. 노이즈를 지우려다 사실을 지우는 것이고 C-24·C-25 가 겨냥한 실패와 같은 종류다. 그렇다고 게이트를 안 두면 근거 0 요약이 5인에게 나간다(C-18 `:91` 의 실호출 실패). **$9.61 을 태우고 얻었다** — `noise-filter` 2차 설계가 확장 쪽으로 갔다가 judge #37 반박 → impl BLOCKED. 두 사실은 서로를 덮지 않는다: `no_change: false` + `watched_files` 전부 `skipped` + `change_stats.files_changed: 0`. 상세는 JOURNAL (카) |
...
.venv\Scripts\python -m pytest -q       # 502 passed 나와야 정상 (2026-09-09 기준)
