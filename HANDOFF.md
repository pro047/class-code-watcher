...
- 게이트 3종 녹색: 268 passed / ruff clean / mypy 15 files
  (2026-08-30 macOS·Python 3.12.11 / **2026-08-31 Windows·Python 3.14.6** 양쪽 실측)
- 한 줄 요약: **0~4단계 코드 완료 — 테스트 268개, 게이트 3종 녹색.
  파이프라인이 baseline→감시→diff→정제→요약(`summary.json`)까지 이어졌다.
  **2026-08-31 실기기 검증 라운드에서 2·3·4단계가 전부 닫혔다** — 2단계 5/5, 3단계 2/2,
  **4단계 6/6**. 1단계만 4번(Z: 드라이브 E2E)·6번(자원)이 남았고 둘 다 진행을 안 막는다.
  이제 바로 5단계 "Discord"(0.5일) 주행이다.**
...
| 단계 | 이전 | 지금 | 남은 것 |
|---|---|---|---|
| 1 | 4/6 | **4.5/6** | 4번의 Z: 드라이브 E2E(학원 PC), 6번 자원 사용 |
| 2 | 3/5 | **5/5 ✅** | — |
| 3 | 1/2 | **2/2 ✅** | — |
| 4 | 2/6 | **6/6 ✅** | — |
...
### (가) 사람만 할 수 있는 것 — 실기기 확인 (1단계 4.5/6, **2단계 5/5 · 3단계 2/2 · 4단계 6/6 완료**)
...
6. ⏳ **자원 사용** — 200파일 1시간 감시에 CPU 2% 이하 / 메모리 150MB 이하.
   **2026-08-31 측정 진행 중** (결과는 아래 표에 채운다).
   ...
   | 항목 | 실측 | 기준 |
   |---|---|---|
   | 기동 직후 WS(3프로세스 합) | **77.7MB** (피크 78.4MB) | 150MB 이하 |
   | 기동 CPU | 0.98초 | — |
   | 1시간 평균 CPU | *(측정 중)* | 2% 이하 |
   | 1시간 뒤 WS 증가 | *(측정 중)* | 누수 신호 |

   > **기동만으로 이미 78MB 다.** 150MB 기준의 절반을 시작하자마자 쓴다 —
   > 여유가 생각보다 없다. 이 수치가 3단 프로세스 합이라는 점도 같이 봐야 한다
   > (7단계 단일 exe 로 가면 구조가 달라진다).
