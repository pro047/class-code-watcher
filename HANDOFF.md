...
- 기준 문서: `PRD.md` **v1.4** (14절 MVP 단계별 개발 계획, C-17·C-18·C-19 반영)
- 갱신 시점: 2026-09-01
...
- 한 줄 요약: **0~5단계 완료. C-17(메시지 형식)·C-18(하루치 예산)은 코드에 들어갔고,
  ...
  **`dynamic-groups` 로 시작해 design 까지 끝내고 judge 중간에 중단했다 — 5절 (아).**
  재개 전에 **사람이 A/B 를 정해야 한다**: 설계가 1건짜리 묶음 병합을 넣었는데
  주행 직전 사람 결정은 「핵심만」이었다. **소스는 한 줄도 안 바뀌었다**,
...
#### ⚠ 재개 전에 반드시 읽어라 — 사람 결정과 설계가 어긋나 있다

**사람은 주행 직전에 범위를 「핵심만, 1건짜리 묶음 병합은 다음 주행」으로 정했다.
그런데 `DESIGN.md` 는 병합을 넣었다.** 설계가 규칙을 어긴 것이 아니다 —
**PRD 11.3 의 「개수」 행에 「1건짜리 묶음 병합은 렌더러가 한다」가 이미 정본으로
있고** 설계가 그것을 인용했다(`DESIGN.md:46`).
...
**B 를 고르면서 PRD 를 안 고치면 같은 자리에서 또 부딪힌다.** 이것이 이 어긋남의 핵심이다.
...
- **병합한 묶음의 이름을 PRD 가 정하지 않았다.** 11.3 은 「병합한다」까지만 말한다.
  렌더러는 의미를 모르므로 모델 제목 하나를 고르면 **나머지가 거짓 라벨이 된다 —
  C-19 가 없애려던 바로 그 실패다.** 설계는 `그 외` 라벨을 새로 두는 것을 유일하게
  거짓말하지 않는 선택으로 판정했다(`DESIGN.md:102`).
...
```bash
# 산출물은 .pipeline/dynamic-groups/ 에 그대로 있다 (커밋되지 않는다 — 2.1 절)
cat .pipeline/dynamic-groups/DESIGN.md          # 713줄. 먼저 읽어라

# A 안 — 설계 그대로 간다
./approve.sh dynamic-groups DESIGN.md
PY=.venv/bin/python ./orchestrate.sh dynamic-groups   # judge 부터 다시 돈다

# B 안 — 병합을 뺀 설계를 다시 뽑는다 (PRD 11.3 개정이 먼저다)
FRESH_DESIGN=1 PY=.venv/bin/python ./orchestrate.sh dynamic-groups
```

**`DESIGN.md` 는 승인 마커가 없다** — `.approved` 파일이 없으므로 재실행하면 게이트에서
다시 선다. 중단이 남긴 것은 설계 한 벌과 judge 스트림 조각뿐이고, **소스는 한 줄도
안 바뀌었다** (`git status` 깨끗). 되돌릴 것이 없다.
