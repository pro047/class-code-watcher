...
- 🔴 **게이트 3종이 지금 빨갛다 — 의도된 중간 상태다.**
  **319 passed / 18 failed / 42 수집불가** · ruff clean · mypy 17 files
  (2026-09-01 macOS·3.12.11 실측). 소스는 새 형식으로 갈렸는데 **테스트가 아직
  옛 스키마**여서 그렇다. `prompts/impl.md` 가 impl 에게 테스트 수정 권한을 안 주고
  verify 에만 주는 분업의 결과다. **직전 녹색 기준선은 379 passed** (`975693b`).
  **재개 방법은 5절 (바) 의 「재개 방법」에 있다** — `mv` 한 줄 + 재실행이다.
- 한 줄 요약: **0~5단계 완료 + 메시지 형식 교체가 소스까지 끝났다 (테스트 미완).**
  ...
  **다음 작업은 순서가 정해져 있다** — (1) `note-format` verify 재개(5절 바),
  (2) **`prompt-budget` 주행 — 하루치 예산·키워드 상한·절단 결함**(5절 사, C-18),
...
### (바) 메시지 형식 교체 — 🔶 **소스 구현 완료 / 테스트 미완** (2026-08-31~09-01)

> **여기부터 읽어라 — 현재 상태 (2026-09-01).**
>
> | | |
> |---|---|
> | `src/class_watcher/summarize.py`·`notify.py` | ✅ **교체 완료**. ruff·mypy 초록 |
> | `tests/` 4파일 | ❌ **옛 스키마 그대로.** verify 가 아직 안 돌았다 |
> | `pytest` | 🔴 **319 passed / 18 failed / 42 수집불가** (기준선 379) |
> | 파이프라인 | `.pipeline/note-format/` 이 `IMPL.md.crashed` 파킹 게이트에 멈춰 있다 |
>
> **이 커밋의 pytest 는 빨갛다. 결함이 아니라 분업의 중간 상태다** —
> `prompts/impl.md` 가 impl 에게 테스트 수정 권한을 안 주고 `prompts/verify.md:7` 이
> verify 에만 준다. impl 은 일부러 빨간 게이트를 남기고 인계한다 (하네스 결함 ⑤ 예외).
>
> **재개 방법**은 이 절 맨 아래 「주행 실적」을 보라.
>
...
### (사) 하루치 예산 — 🔶 **결정 완료 (C-18) / 구현 미착수** (2026-09-01)
