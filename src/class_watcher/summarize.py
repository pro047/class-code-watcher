...
# C-27 로 60,000 에서 올렸다. C-18 의 "하루치 약 40,000자"는 `추정`이었고, 2026-09-08
# 실수업 세션(JS DOM, 8시간)이 그 추정을 깼다 — final.diff 108,957자로 예산의 181.6%.
# 110,000 은 그 실측에 여유를 얹지 않은 값이다: 하루치 상한을 실측 하나로 못 박고,
# 다음 초과가 나오면 다시 실측으로 올린다. PRD 7절의 밀도 2.98자/토큰으로 약 37k 토큰.
# 조정은 이 한 줄로 한다.
PROMPT_DIFF_BUDGET_CHARS = 110_000
