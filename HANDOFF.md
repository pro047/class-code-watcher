...
- 게이트 3종 녹색: **497 passed** / ruff clean / mypy 17 files
...
| **F17** | **C-24 가 파일 단위라 공백 노이즈가 줄 단위로 살아남는다.** 진짜 변경이 섞인 파일에서는 재들여쓰기가 `final.diff` 에 그대로 실린다 — 2026-09-08 오전 세션에서 **diff 의 47%**. **절단의 진짜 원인이고 F1·C-27 보다 효과가 크다** — 아래 별도 항목 | `diffgen` |
...
| 공백 접은 diff | 37,153자 (예산의 **62%**) |

**공백 줄만 걷어내면 69,752 → 37,153 자다. 그 세션은 안 잘렸다** — 13/17 hunk 가 아니라
17/17 이었다. 실제 예시:
...
**우선순위**: 절단을 없애는 효과가 C-27(배분)보다 크다. 배분은 「잘린 것을 어떻게
나눌까」이고 F17 은 「잘릴 일 자체를 없앤다」이다. 다만 **층이 `diffgen` 이라
C-24 개정(FR-025)이 따라붙는다** — 파이프라인 안에서 닫을 후보다.

**주의 — 다 없애지는 못한다.** C-24 근거 문단이 이미 적었다: 대소문자
(`<!DOCTYPE>`→`<!doctype>`)·자기닫힘 태그·**줄 접힘**은 공백이 아니라 그대로 남는다.
47%가 0%가 되는 것이 아니다.

**그때까지의 운영 회피책**: 수업 폴더의 `editor.formatOnSave` 를 끄거나, 수업 시작 전에
전체를 한 번 저장해 포맷을 정착시킨 뒤 `cw` 를 띄운다. 아래 「운영 회피책」과 같은 것이다.
...
.venv\Scripts\python -m pytest -q       # 497 passed 나와야 정상 (2026-09-08 기준)
