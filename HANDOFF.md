...
- 한 줄 요약: **0~5단계 완료. C-17(메시지 형식)·C-18(하루치 예산)은 코드에 들어갔고,
  C-19(동적 분류 제목)는 PRD v1.4 로 확정됐지만 아직 코드에 없다.**
  파이프라인이 baseline→감시→diff→정제→요약→Discord 전송까지 **실물로** 이어졌다.
  **실전송 3회 전부 HTTP 204** (08-31, 09-01 오전 18건/7묶음/1,622자, 09-01 오후 19건/2묶음/1,852자).
  `추정` 3건이 닫혔고 5단계 사람 확인은 4/8 (A·E·F·G) 통과, B 는 자연 트리거 불가로 판명.
  남은 것은 **C(모바일)·D(수신자 5인)** — 둘 다 사람만 할 수 있고 **C-19 가 코드에 들어간
  뒤에 해야 한다** (지금 채널에 있는 09-01 오후 메시지는 분류 라벨이 거짓이라, 그대로 물으면
  답이 F9 에 오염된다 — 5절 (사) 「재전송 실적」). **D 가 이 제품의 최종 합격선이다** (PRD 15절).
  1~4단계 실기기도 사실상 닫혔다(2단계 5/5, 3단계 2/2, 4단계 6/6, 1단계 5.5/6).
  **다음 작업 순서 — 2026-09-01 사람이 정했다**:
  (1) **`.env` 모델 승급 `OPENAI_MODEL=gpt-4o`** (F8) — 사람 결정 완료, `.env.example`
  반영 완료. **각 PC 의 `.env` 에 한 줄 넣는 것만 남았다** (에이전트는 `.env` 를 못 건드린다).
