...
- 한 줄 요약: **0~5단계 완료 — 테스트 379개, 게이트 3종 녹색.
  ...
  **2026-08-31 실전송 성공 (HTTP 204).** `추정` 3건이 닫혔고 5단계 사람 확인은 4/8
  ...
  — 둘 다 사람만 할 수 있다. **D 가 이 제품의 최종 합격선이다** (PRD 15절).
  1~4단계 실기기도 사실상 닫혔다(2단계 5/5, 3단계 2/2, 4단계 6/6, 1단계 5.5/6).
  **다음 작업은 두 갈래다** — (1) 메시지 형식 교체(5절 바, 결정 완료·구현 미착수,
  `PRD.md` 개정이 선행), (2) 6단계 통합.**
...
### (바) 메시지 형식 교체 — 결정됨, 구현 미착수 (2026-08-31)

**실수업 세션 1회로 형식을 확정했다.** 코드는 아직 안 바꿨다 — 프로토타입으로만 검증했다.
...
| 파일 | 변경 | 단계 |
|---|---|---|
| `PRD.md` 11.3 | 응답 스키마: `changes`·`learning_points` → `keywords{term, concept, syntax, group, confidence}` | **사람** (PROTECTED) |
| `PRD.md` 11.4 | 메시지 예시를 위 템플릿으로 교체 | **사람** (PROTECTED) |
...
> **`PRD.md` 개정이 선행돼야 한다.** PROTECTED 라 에이전트가 못 건드리고
> 셸이 지문으로 감시한다(8절). 사람이 먼저 11.3·11.4 를 고친 뒤 주행해야
> 설계가 정본을 인용할 수 있다.
...
**1단계 — `PRD.md` 개정 (사람, 선행 필수)**

11.3 응답 스키마와 11.4 메시지 예시를 위 템플릿으로 고친다. `PRD.md` 는
`PROTECTED` 라 에이전트가 못 건드리고 셸이 지문으로 감시한다(8절).
**설계가 인용할 정본이 없으면 judge 가 "PRD 에 근거가 없다"로 반박한다** —
5단계에서 실제로 그런 반박이 여러 건 나왔다.
...
- **`MAX_ARRAY_ITEMS`(=5) 는 그대로 둔다** (위 결정). 상한을 올리자는 설계가 나오면
  게이트에서 반려해라.
- **`evidence` 필드 처리**를 다시 정해야 한다. 판정 9번 ③ 이 "스키마에 남기고
  메시지에서만 뺀다"로 정했는데, 새 스키마에는 `changes` 자체가 없다.
  `keywords` 에 `evidence` 를 둘지, 아니면 이 참에 뺄지가 **새 판정 지점**이다.
