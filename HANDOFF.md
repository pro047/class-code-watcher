...
- 게이트 3종 녹색: **379 passed** / ruff clean / **mypy 17 files**
  (2026-08-31 Windows·Python 3.14.6 실측. 4단계까지는 macOS·3.12.11 에서도 실측했다)
- 한 줄 요약: **0~5단계 코드 완료 — 테스트 379개, 게이트 3종 녹색.
  파이프라인이 baseline→감시→diff→정제→요약→**Discord 렌더링·전송**까지 이어졌다.
  1~4단계 실기기 확인도 사실상 닫혔다(2단계 5/5, 3단계 2/2, 4단계 6/6, 1단계 5.5/6).
  **다음은 5단계 사람 확인 8건이고, 그 중 A(실전송 1회)가 `추정` 3건을 동시에 닫는다** —
  그 전까지 "Discord 전송이 실제로 동작한다"고 말하면 안 된다 (5절 가).
  `.env` 의 `DISCORD_WEBHOOK_URL` 이 비어 있어 A 를 아직 못 돌린다.**
...
| *(미커밋)* | **5단계 Discord — `notify.py`·`discord_client.py` 신규, 테스트 379개** + `orchestrate.sh` 상한 조정 |
...
| **5. Discord** | 메시지 렌더링, 모바일 가독성, Webhook, 실패 보존 | 204/4xx/5xx mock 테스트 | 0.5일 | ✅ **코드 완료** (2026-08-31) — **실전송 0회**. 사람 확인 8건 전부 미확인 |
...
1. ⬜ **A. 실전송 1회** — `.env` 에 실 `DISCORD_WEBHOOK_URL`·`OPENAI_API_KEY` 를 넣고
   2~3파일 수업 규모 세션을 돌린 뒤 Ctrl+C 한 번.
   기대: 콘솔 `[DONE] 요약과 Discord 전송을 완료했습니다`, 종료 코드 **0**,
   채널에 메시지가 **본문 텍스트로** 보인다.
   **`session.json` 의 `discord.http_status` 를 적어 둬라** — 204 가 아니면
   `discord_client.is_success` 의 2xx 판정을 다시 본다.
   413/400 이 오면 `notify.DISCORD_CONTENT_LIMIT` 이 틀린 것이고, 그 값을 고치면
   `test_first_screen_budget_fits_in_one_chunk` 가 **즉시 깨져 알려 준다**(설계 케이스 7-1).
2. ⬜ **B. 분할 2건** — 파일 5개 이상·긴 설명으로 2조각을 만든다.
   기대: 채널에 메시지 2개, 각 첫 줄이 `(1/2) `·`(2/2) `,
   `discord_payload.json` 의 `chunks == 2`. 순서가 뒤집히면 `MAX_CHUNKS` 를 1 로 검토.
...
5. ⬜ **E. cp949 리다이렉트** — `class-watcher watch . > log.txt` 후 Ctrl+C.
   기대: `UnicodeEncodeError` 없이 종료, `log.txt` 에 요약 전문과 `[정리 안내]` 3줄.
   **모델이 이모지를 준 세션에서 특히 봐야 한다** — 템플릿은 이미 안전 문자만 쓰지만
   (설계 케이스 7-4) 모델 문자열은 `console_safe` 가 `?` 로 바꿔야 한다.
6. ⬜ **F. 잘못된 Webhook URL** — URL 뒤에 아무 문자나 붙여 돌린다.
   기대: stderr 에 `[FAILED] Discord 전송에 실패했습니다.` + `discord_payload.json`
   절대 경로, 종료 코드 1, `status: "partial"` / `error: "discord_http_<코드>"`.
   **URL 이 콘솔·`errors.jsonl`·`discord_payload.json` 어디에도 없어야 한다**
   (판정 지점 3번의 실물 검증).
7. ⬜ **G. 전송 도중 두 번째 Ctrl+C** — B 의 2조각 세션에서 전송 시작 직후 한 번 더.
   기대: 종료 코드 **130**, `status: "failed"`, **`discord_payload.json` 은 남는다**
   (전송 전에 쓰므로). 이미 나간 조각은 채널에 남는다 — 설계가 수용한 동작이다.
