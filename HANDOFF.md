...
- 게이트 3종 녹색: **422 passed** / ruff clean / mypy 17 files
  (2026-09-01 Windows·Python 3.14.6 실측. **같은 날 macOS·Python 3.12 에서 재확인 — 세 값 모두 동일**).
  **`note-format` 이 남긴 빨간 상태 (319/18/42)는 `prompt-budget` verify 가 함께 닫았다** —
  테스트가 379 → 422 개가 됐다.
- 한 줄 요약: **0~5단계 완료. C-17(메시지 형식)·C-18(하루치 예산)은 코드에 들어갔고,
  C-19(동적 분류 제목)는 PRD v1.6 으로 확정됐지만 아직 코드에 없다.**
  ...
  **09-02 C-19 형식 28건/8묶음/2,664자·2청크 — 제품이 아니라 스크립트가 보냈다**, 5절 (자)).
  ...
  남은 것은 **C(모바일)·D(수신자 5인)** — 둘 다 사람만 할 수 있다. **09-02 채널 메시지는
  C-19 형식이고 오분류가 없다 — C·D 를 이 메시지로 물어도 된다.** 09-01 오후 메시지(거짓 라벨)로
  물으면 답이 F9 에 오염된다. **D 가 이 제품의 최종 합격선이다** (PRD 15절).
  1~4단계 실기기도 사실상 닫혔다(2단계 5/5, 3단계 2/2, 4단계 6/6, 1단계 5.5/6).

  **다음 작업 순서 — 2026-09-02 사람이 갱신했다. (1) 이 최우선이다**:
  (1) **★ C-19 주행 — `FRESH_DESIGN=1` 로 설계부터 다시 뽑는다** (`summarize.py`·`notify.py`).
  **사람이 정할 것 3건은 전부 닫혔다 (2026-09-02):** ① judge 반박 #28 은 C-23 이
  `MAX_TERM_CHARS` 를 **16** 으로 다시 정해 논점 자체가 사라졌고, ② F14 → **C-22**,
  ③ F15 → **C-23** 으로 PRD v1.7 에 들어갔다.
  **그래서 기존 `DESIGN.md`·`JUDGE.md` 는 폐기다** — 둘 다 v1.6 을 정본으로 삼았고,
  C-23 이 `term`·`syntax` 상한과 상한 초과 clamp 순서를 바꿔 **설계 §6 예산식과 §3 변경
  목록이 통째로 어긋난다.** 재사용하면 낡은 설계로 조용히 돈다 — `FRESH_DESIGN=1` 이 필수다.
  `docs/pipeline-transfer/dynamic-groups/` 의 사본은 **더 이상 재개 자료가 아니다**;
  judge 판정 기록으로만 남긴다 (5절 아). **소스는 아직 한 줄도 안 바뀌었다**,
  (2) **하루치 세션 1회** → H1·H3·H4 + 사람 확인 C·D 를 한자리에서 회수 (5절 사. H2 는 닫혔다),
  (3) **F6·F7 주행** — 포맷터 노이즈 필터 + atime 오탐 (`diffgen`+`watcher`. 같은 뿌리다),
  (4) 6단계 통합.
...
| *(이 커밋)* | **C-19 주행 `dynamic-groups` 중단 기록 (5절 아)** — design DONE / judge 중단. 설계가 범위 결정과 어긋난다 |
...
### (아) C-19 주행 — `dynamic-groups` **중단됨** (2026-09-01, macOS)
