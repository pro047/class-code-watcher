...
- 게이트 3종 녹색: 268 passed / ruff clean / mypy 15 files (2026-08-30 실측)
- 한 줄 요약: **0~4단계 코드 완료 — 테스트 268개, 게이트 3종 녹색.
  파이프라인이 baseline→감시→diff→정제→요약(`summary.json`)까지 이어졌다.
  **실 OpenAI 호출 1회 성공**(2026-08-30), 3단계 실기기 1/2·4단계 실 API 2/6 통과.
  5단계 진입 전 사람 몫이던 프롬프트 문안 2건은 닫혔다 — 이제 바로 5단계
  "Discord"(0.5일) 주행이다.**
...
6. 🔶 **요약 가독성 — 부분 통과** (FR-032, PRD 14.1 마지막). **이 제품의 진짜 합격선이다.**
   ...
   **아직 실호출로 확인 안 됐다** — 다음 실 세션에서 `summary.json` 의
   `questions_to_review` 가 비어 있지 않은지, `evidence` 가 `file` 과 다른 값인지 본다.
   여전히 빈 배열이면 프롬프트로는 부족하다는 뜻이고, 그때 스키마 `minItems` 를
   실측 근거와 함께 다시 논의한다 (지금은 넣지 않았다 — 판정 지점 9번 아래 근거).
