...
### (가) 사람만 할 수 있는 것 — 실기기 확인 (1단계 4/6, 2단계 3/5, **3단계 1/2, 4단계 2/6**)
...
3. ⬜ **strict 가 maxItems/maxLength 를 강제하는가** (JUDGE 미확인 #26) — **아직 미확정.**
   실호출 세션의 배열이 2·2개라 5개 상한을 안 건드렸다. 어느 쪽이든 로컬 clamp 가 같은
   결과를 내므로 급하지 않다. **큰 diff 세션에서 자연히 드러난다**
4. ⬜ **입력 20,000자 ≈ 8k 토큰인가** (JUDGE 미확인 #27) — **아직 미확정.** 실호출 입력이
   `diff_chars: 453` 이라 표본이 못 된다. **한글 주석 비중이 큰 diff 는 밀도가 더 높다.**
   복구는 `PROMPT_DIFF_BUDGET_CHARS` 상수 1곳
5. ⬜ **timeout 15초 실동작** — SDK 기본이 **600초**다(judge 실측). 명시가 안 먹으면
   세션 종료가 10분 멈춘다. 네트워크를 끊고 세션을 종료해 15초 안에 실패하는지 본다
6. 🔶 **요약 가독성 — 부분 통과** (FR-032, PRD 14.1 마지막). **이 제품의 진짜 합격선이다.**
   ...
   **미해결 — 프롬프트 문안 수정이 필요하다.** `summarize.py` 의 system/user 프롬프트
   상수 2곳이면 되고 스키마·검증·호출 경로는 안 건드린다. **다음 세션 몫으로 남긴다.**
...
9. **`evidence` 필드를 유지할 것인가** *(같은 검증에서 나왔다)* — PRD 11.3 응답 스키마에는
   ...
   **설계가 고르고 근거를 적어야 한다** — 지금은 아무도 안 정한 채 비용만 나가고 있다
...
**프롬프트 문안 수정** — 5(가) 4단계 ⑥번에서 나온 2건이다. `summarize.py` 의 system/user
프롬프트 상수만 고치면 되고 스키마·검증·호출 경로는 안 건드린다. 파이프라인 한 바퀴를
돌릴 일이 아니다.

- `questions_to_review` 를 **반드시 채우게** 한다 (지금은 빈 배열로 와서 섹션이 사라진다)
- `evidence` 에 무엇을 적을지 명시한다 — 판정 지점 9번이 정해지면 그에 맞춘다

**순서 주의**: 9번(evidence 유지 여부)이 정해지기 전에 프롬프트를 고치면 두 번 고친다.
